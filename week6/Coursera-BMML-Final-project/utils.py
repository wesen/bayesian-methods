...
def create_encoder(input_dims, base_filters=64, layers=4, latent=512):
    ...
    for i in range(layers):
        encoder.add(
            Conv2D(filters=base_filters * 2 ** i, kernel_size=(5, 5), strides=(2, 2), padding='same', bias=False))
...
def create_decoder(output_dims, base_filters=64, layers=4, latent=512):
    ...
    for i in range(layers - 1, 0, -1):
        decoder.add(
            Deconv2D(filters=base_filters * 2 ** i, kernel_size=(5, 5), strides=(2, 2), padding='same', bias=False))
